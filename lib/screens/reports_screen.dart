...
class _ReportsScreenState extends State<ReportsScreen>
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Erro ao carregar relatórios.')),
        );
      }
    }
  }

  List<Appointment> _appointmentsByDay(DateTime day) {
    final start = DateTime(day.year, day.month, day.day);
    final end = start.add(const Duration(days: 1));

    final list = _allAppointments.where((a) {
  ...
  List<Appointment> _appointmentsByWeek(DateTime anyDayOfWeek) {
  ...
  List<Appointment> _appointmentsByMonth(int year, int month) {
  ...
  double _revenueForDay(DateTime day) => _appointmentsByDay(day)
      .where((a) => a.status == 'completed')
      .fold(0, (sum, a) => sum + a.value);

  double _revenueForWeek(DateTime anyDay) => _appointmentsByWeek(anyDay)
      .where((a) => a.status == 'completed')
      .fold(0, (sum, a) => sum + a.value);

  double _revenueForMonth(int year, int month) =>
      _appointmentsByMonth(year, month)
          .where((a) => a.status == 'completed')
          .fold(0, (sum, a) => sum + a.value);
  ...
  Widget _buildDailyReport() {
    final appts = _appointmentsByDay(_selectedDay);
  ...
  Widget _buildWeeklyReport() {
    ...
    final appts = _appointmentsByWeek(_selectedWeek);
    final completed = appts.where((a) => a.status == 'completed').toList();
    final cancelled = appts.where((a) => a.status == 'cancelled').toList();
    ...
    final prevRevenue = _revenueForWeek(prevWeek);
      ...
      child: Column(
        ...
        children: [
          ...
          ...List.generate(7, (i) {
            final d = days[i];
            final dayCompleted =
                completed.where((a) => _sameDay(a.dateTime, d)).toList();
            if (dayCompleted.isEmpty) return const SizedBox.shrink();
            return _buildDayBlock(
              dayLabels[i],
              d,
              dayCompleted,
              dailyRevenues[i],
          ...
          if (completed.isEmpty)
            _emptyState('Nenhum atendimento concluído nesta semana'),
        ],
      ),
    );
  }
  ...
  Widget _buildMonthlyReport() {
    ...
    final appts = _appointmentsByMonth(year, month);
    final completed = appts.where((a) => a.status == 'completed').toList();
    final cancelled = appts.where((a) => a.status == 'cancelled').toList();
    ...
    final prevRevenue = _revenueForMonth(prevMonth.year, prevMonth.month);
  ...
  Widget _navContainer({
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: AppTheme.rosePrimary.withOpacity(0.08),
            blurRadius: 8,
      ...
      child: Row(
        ...
        children: [
          IconButton(
            icon: const Icon(Icons.chevron_left, color: AppTheme.rosePrimary),
            onPressed: onPrev,
          ),
          GestureDetector(
            onTap: onTap,
          ...
          IconButton(
            icon: const Icon(Icons.chevron_right, color: AppTheme.rosePrimary),
            onPressed: onNext,
          ),
        ],
      ),
  ...
  Widget _buildFinancialHero({
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: AppTheme.rosePrimary.withOpacity(0.35),
            blurRadius: 16,
  ...
  Widget _financialRow(Appointment a, {bool pending = false}) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border:
            pending ? Border.all(color: AppTheme.gold.withOpacity(0.3)) : null,
        boxShadow: [
          BoxShadow(
            color: AppTheme.rosePrimary.withOpacity(0.06),
            blurRadius: 6,
            offset: const Offset(0, 2),
          ),
        ],
  ...
  Widget _buildDayBlock(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: AppTheme.rosePrimary.withOpacity(0.07),
            blurRadius: 8,
            offset: const Offset(0, 2),
          ),
        ],
        ...
        children: [
          Container(
            ...
            child: Row(
              ...
              children: [
                Text(
                  '$dayLabel — ${DateFormat('d/MM', 'pt_BR').format(day)}',
                  style: GoogleFonts.lato(
                    fontWeight: FontWeight.w700,
                    color: AppTheme.rosePrimary,
                  ),
          ...
          Padding(
            ...
            child:
                Column(children: appts.map((a) => _financialRow(a)).toList()),
          ),
        ],
      ),
    );
              ...
              child: Row(
                children: [
                  Expanded(
                    child: Text(
                      'Semana ${r['week']}',
                      style: GoogleFonts.lato(color: AppTheme.textDark),
                    ),
                  ),
                  Text(
                    '${r['count']}x',
                  ...
                  SizedBox(
                    width: 100,
                    child: Text(
                      'R\$ ${(r['revenue'] as double).toStringAsFixed(2)}',
                      textAlign: TextAlign.right,
                      style: GoogleFonts.lato(
                        color: AppTheme.gold,
                        fontWeight: FontWeight.bold,
        ...
        BarChartData(
          ...
          gridData: FlGridData(
            ...
            getDrawingHorizontalLine: (_) =>
                FlLine(color: AppTheme.roseLight, strokeWidth: 1),
  ...
  Widget _buildLineChart(List<double> dailyRevs, int daysInMonth) {
    ...
    for (int i = 0; i < dailyRevs.length; i++) {
      if (dailyRevs[i] > 0) {
        spots.add(FlSpot(i.toDouble() + 1, dailyRevs[i]));
      }
    }
          ...
          child: Column(
            children: [
              Icon(Icons.bar_chart_outlined,
                  size: 48, color: AppTheme.roseLight),
              const SizedBox(height: 10),
              Text(
                msg,
                style: GoogleFonts.lato(
